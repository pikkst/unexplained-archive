# ⚠️ KREDIIDI SÜSTEEMI MANUAALNE RAKENDAMINE

Kuna migratsioonide versioning konflikt, rakenda krediidi süsteem käsitsi:
...
### 2. Kopeeri ja käivita see SQL:

```sql
-- User Credits System - Virtual currency for promotional benefits
-- This is separate from wallet balance (real money)
...
CREATE TABLE IF NOT EXISTS credit_transactions (
    ...
    transaction_type VARCHAR(50) NOT NULL, -- 'earned', 'spent', 'promo', 'reward'
    source VARCHAR(100), -- 'campaign_redemption', 'ai_generation', 'case_donation', 'admin_grant'
...
-- 5. Function to add credits (atomic operation)
...
-- 6. Function to spend credits (with balance check)
...
-- 7. Update campaign redemption to grant credits automatically
...
BEGIN
    -- Extract free_credits from benefit_value
    ...
    IF v_credits IS NOT NULL AND v_credits > 0 THEN
        ...
        PERFORM add_user_credits(
            ...
            'promo',
            'campaign_redemption',
            'Promotional credits from: ' || v_campaign_name,
            NEW.id,
            NULL
        );
    END IF;
...
-- 8. Function to get user credit balance
...
-- 9. Admin function to grant credits manually
...
-- 11. Create view for credit balance summary
...
-- ✅ VALMIS! Krediidi süsteem rakendatud!
```
...
WHERE proname IN ('add_user_credits', 'spend_user_credits', 'get_user_credits', 'admin_grant_credits');
```
...
-- Test get credits (replace with your user ID)
...
Nüüd saad kasutada krediiti süsteemi:
- ✨ Kasutajad saavad krediite promo koodidest
- 💳 Krediite saab kulutada AI genereerimiseks
- 📊 Kõik tehingud logitakse
- 👨‍💼 Admin saab käsitsi krediite anda

---

## 🚨 Kui midagi ei tööta:

Kirjuta veateade ja ma aitan parandada!
